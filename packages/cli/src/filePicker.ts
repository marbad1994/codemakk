...
const ignoredDirs = new Set([".git", "node_modules", "dist", "build", ".next", ".turbo", ".cache", ".codemakk"]);
const ignoredFiles = new Set(["package-lock.json", "pnpm-lock.yaml", "yarn.lock"]);

function isIgnoredPath(filePath: string): boolean {
  const parts = filePath.split(path.sep);
  return parts.some((part) => ignoredDirs.has(part)) || ignoredFiles.has(path.basename(filePath));
}

export async function listFilesRecursive(root: string, maxFiles = 500): Promise<string[]> {
...
export function getAtToken(value: string): { start: number; end: number; query: string } | null {
...
const files = await listFilesRecursive(workingDir);
  return files
    ...
    .slice(0, 20)
    .map((filePath) => ({ type: "file", filePath, isDirectory: filePath.endsWith("/") }));
}
